...
The tool shapes the hand that wields it. And the tools being built right now are shaping us into something we may not recognize.
...
## Section 1: The Two Trajectories
...
## Section 2: What the Bubble Teaches Us
...
## Section 3: What Klarna Teaches Us

Klarna's story is told as an AI success. We read it differently.

**Phase 1:** Deploy AI to replace 700 human agents. Celebrate efficiency gains.

**Phase 2:** Realize something is missing. Customers handled efficiently still feel uncared for.

**Phase 3:** Bring humans back for complex and emotional situations. The hybrid system outperforms pure AI.
...
## Section 4: What Palantir Teaches Us
...
## Section 6: What We're Building Instead
...
### For Builders: Decision Framework

| Decision Point | Extraction Trajectory | Flourishing Trajectory |
|----------------|----------------------|------------------------|
| Data architecture | Cloud-dependent, harvestable | Local-first, private |
| Model choice | Proprietary APIs | Open source, optimized |
| Metrics | Engagement, time-on-app | Value delivered, connection |
| Human role | Replaced by AI | Freed by AI for presence |
| Failure mode | Silent degradation | Graceful handoff to humans |
| Funding | Any money, any partner | Values-aligned only |

### Flourishing-Trajectory Tech Stack

```
DATA LAYER:
├── Local-first database (SQLite, LevelDB)
├── Sync engine (optional, user-controlled)
├── Encrypted at rest (user key)
└── No server-side persistence of user data

INFERENCE LAYER:
├── On-device models (ONNX, CoreML, llama.cpp)
├── Optimized for commodity hardware
├── Open source models only
└── No cloud inference dependencies

SYNC LAYER (if needed):
...
HUMAN HANDOFF:
├── Graceful degradation when AI insufficient
...
**Week 1: Audit**
- Inventory all cloud dependencies
- Identify what data leaves devices
- List proprietary APIs in use
- Document human-AI handoff points (or lack thereof)

**Weeks 2-4: Plan**
- Design local-first alternatives for each cloud dependency
- Identify open-source model replacements
- Architect graceful degradation paths
- Define "presence moments" requiring humans

**Months 2-3: Prototype**
- Build local-first proof of concept
- Test on minimum-spec hardware
- Implement one human handoff pathway
- Measure: Does this serve users or extract from them?

**Ongoing: Iterate**
- Progressively move functions local
- Remove cloud dependencies one by one
- Build optimization muscle
...
## Conclusion: The Line Must Be Drawn Here
...
- [Evoke Passion Ideology Statement](../framework/evoke-passion-statement.md)
